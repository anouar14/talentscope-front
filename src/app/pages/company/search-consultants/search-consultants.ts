...
export class SearchConsultants implements OnInit {
  ...
  filterConsultants(): void {
    ...
    this.filteredConsultants = this.consultants.filter(consultant =>
      ...
      (consultant.location ?? '').toLowerCase().includes(term) ||
  ...
  launchMatching(): void {
    ...
    const normalizedDescription = this.jobDescription.trim();
    ...
    this.companyService.matchConsultants(normalizedDescription).subscribe({
      next: response => {
        this.matchingResults = (response.results ?? []).map(result => ({
          ...result,
        ...
        this.analyzedConsultants = response.analyzedConsultants ?? 0;
  ...
  getScoreCriteria(result: ConsultantMatchResult): ScoreCriterion[] {
    ...
    return [
      {
        ...
        available: this.isRequiredSkillsCriterionAvailable(result)
      },
      {
        label: 'Compétences souhaitées',
        score: breakdown.preferredSkillsScore,
        available: this.isOptionalCriterionAvailable(
  ...
  getRecommendation(score: number): RecommendationLevel {
    if (score >= 80) {
      return {
  ...
  isStarActive(star: number, score: number): boolean {
    return star <= this.getRecommendation(score).stars;
  }
  ...
  viewConsultantProfile(consultantId: string): void {
    if (!consultantId) {
      console.error('Identifiant du consultant absent');
      return;
    }
  ...
  trackConsultant(index: number, consultant: Consultant): string {
    return consultant.id;
  }

  trackCriterion(index: number, criterion: ScoreCriterion): string {
    return criterion.label;
  }

  trackStar(index: number, star: number): number {
    return star;
  }
  ...
  private normalizeScoreBreakdown(
    ...
    return {
      requiredSkillsScore: breakdown?.requiredSkillsScore ?? 0,
      preferredSkillsScore: breakdown?.preferredSkillsScore ?? 0,
      experienceScore: breakdown?.experienceScore ?? 0,
      titleScore: breakdown?.titleScore ?? 0,
      locationScore: breakdown?.locationScore ?? 0,
      aiScore: breakdown?.aiScore ?? 0
    };
  }
  ...
  private isRequiredSkillsCriterionAvailable(
    ...
    return result.scoreBreakdown.requiredSkillsScore > 0 ||
      result.matchedSkills.length > 0 ||
      result.missingSkills.length > 0;
  }

  private isOptionalCriterionAvailable(score: number): boolean {
    return score > 0;
  }
}